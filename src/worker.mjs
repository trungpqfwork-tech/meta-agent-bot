import { retrieve, agentPolicy, validateAnswer } from './knowledge.mjs';
  ...
  async process(j) {
    ...
    try {
      if(!j.text.trim()) {log(this.logFile,`process empty_text psid=${j.psid}`);answer={action:'handoff',text:'',sourceIds:[],reason:'unsupported_attachment'};}
        ...
        const payload={page:{name:c.pageName,scope:c.scopeDescription,topics:c.scopeKeywords},documents:docs.map(({score,...d})=>d),history,currentMessage:j.text};
    ...
    } catch { answer={action:'handoff',text:'',sourceIds:[],reason:'agent_or_knowledge_unavailable'}; }
